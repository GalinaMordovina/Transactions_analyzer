...
def test_get_card_stats():
    ...
    assert len(result) == 2
    assert any(
        card["last_digits"] == "*1111" and card["total_spent"] == 300.0
        for card in result
    )
    assert any(
        card["last_digits"] == "*2222" and card["cashback"] == 3.0 for card in result
    )
...
@pytest.mark.skipif(
    not os.path.exists(TEST_FILE), reason="Файл data/operations.xlsx не найден"
)
...
def test_event_view_output(mock_stocks, mock_rates):
    ...
    mock_stocks.return_value = [
        {"stock": "AAPL", "price": 250.0},
        {"stock": "TSLA", "price": 300.0},
    ]
